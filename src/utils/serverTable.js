...
class ServerTable extends React.Component {
    ...
    renderData() {
        ...
        return data.map(function (row, row_index) {
            row.index = row_index;
            return (
                <tr className={'linha'+row_index} key={row_index}>
                    {
    ...
    handleFetchData() {
        ...
        //--- get token -----------
        const tokenx = localStorage.getItem('jwtToken');
        //Headers
        const configx = {
            headers: {
        ...
        if(tokenx){
            configx.headers["Authorization"] = `Token ${tokenx}`;
        }
        axios.get(url + com + urlParams.toString(), configx)
